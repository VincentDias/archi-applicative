...
const score = {
  calculateScoreHorizontal: (gameState) => {
    ...
    for (let row = 0; row < gameState.grid.length; row++) {
      for (let col = 0; col < gameState.grid[row].length - (init.MINIMUM_ALIGNED_TOKENS() - 1); col++) {
  ...
  determineAlignementEtScore: (gameState, lastRowIndex, lastColIndex) => {
    const currentPlayer = gameState.currentTurn;
    ...
    if (
      score.checkAlignment(
        row.map((cell) => cell.owner === currentPlayer),
        lastColIndex
      )
    ) {
      isHorizontal = true;
    ...
    if (
      score.checkAlignment(
        diagonalTLtoBR.map((cell) => cell.owner === currentPlayer),
        diagonalTLtoBR.findIndex((cell) => cell.row === lastRowIndex && cell.col === lastColIndex)
      )
    ) {
      isDiagonal = true;
    }
    ...
    if (
      score.checkAlignment(
        diagonalBLtoTR.map((cell) => cell.owner === currentPlayer),
        diagonalBLtoTR.findIndex((cell) => cell.row === lastRowIndex && cell.col === lastColIndex)
      )
    ) {
      isDiagonal = true;
    }
