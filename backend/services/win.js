...
const endGame = {
  checkWin: (gameState) => {
    let horizontalWinner = endGame.checkWinHorizontal(gameState.grid);
    let verticalWinner = endGame.checkWinVertical(gameState.grid);
    let diagonalWinner = endGame.checkWinDiagonal(gameState.grid);
    ...
    if (!playersHaveRemainingTokens) {
      ...
      victoryType = "score";
      let scoreWinner = endGame.checkScores(gameState);
    ...
    if (winner != null) {
      if (winner !== "draw") {
        ...
        (winner = "draw"), (gameState.gameEndTime = Date.now());
        gameDurationTimestamp = gameState.gameEndTime - gameState.gameStartTime;
        gameDuration = new Date(gameDurationTimestamp).toISOString().substr(11, 8);
        loser = "draw";
        gameType = gameState.gameType;
  ...
  checkWinHorizontal: (grid) => {
    let winner = null;
    const isPlayer1Winner = grid.some((row) => row.every((cell) => cell.owner === "player:1"));

    const isPlayer2Winner = grid.some((row) => row.every((cell) => cell.owner === "player:2"));
  ...
  checkWinVertical: (grid) => {
    let winner = null;
    const isPlayer1Winner = grid[0].map((_, col) => grid.every((row) => row[col].owner === "player:1")).some(Boolean);

    const isPlayer2Winner = grid[0].map((_, col) => grid.every((row) => row[col].owner === "player:2")).some(Boolean);
